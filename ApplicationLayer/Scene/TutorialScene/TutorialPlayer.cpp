...
// シリアルナンバーの静的メンバ初期化
uint32_t TutorialPlayer::nextSerialNumber_ = 0;
...
void TutorialPlayer::Initialize() {
    ...
    // パーティクルエミッターの初期化
    particleEmitter_ = std::make_unique<ParticleEmitter>(particleManager, "PlayerHitParticles");
    // オブジェクトの生成・初期化
...
void TutorialPlayer::Draw() {
    ...
    Vector3 endPos = worldTransform_.translate_ + -direction * 5.0f;
    Wireframe::GetInstance()->DrawLine(worldTransform_.translate_, endPos, { 0.0f, 1.0f, 0.0f, 1.0f });
#endif
}
...
void TutorialPlayer::OnCollision(Collider* other) {
    ...
    if (typeID == static_cast<uint32_t>(CollisionTypeIdDef::kEnemy)) {
        if (!isInvincible_) {
            //hp_ -= 2; // ←コメントアウト維持
            if (hp_ <= 0) {
                // isDead_ = true; // 死亡フラグを立てる（コメントアウト維持）
                isEnemyHit_ = false; // 敵に当たったフラグを解除
            }
