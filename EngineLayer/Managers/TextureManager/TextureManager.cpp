...
DirectX::ScratchImage TextureManager::LoadTextureData(const std::string& filePath)
{
	...
	std::wstring filePathW = LogString::ConvertString(filePath);
...
void TextureManager::LoadTexture(const std::string& filePath)
{
	...
	DirectX::ScratchImage image{};
	std::wstring filePathW = LogString::ConvertString(filePathStr);
	...
	// 中間リソースにデータを転送
	ComPtr<ID3D12Resource> intermediateResource = UploadTextureData(textureData.resource.Get(), mipImages, dxCommon_->GetDevice(), dxCommon_->GetCommandList());
