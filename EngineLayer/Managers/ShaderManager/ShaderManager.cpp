...
Microsoft::WRL::ComPtr <IDxcBlob> ShaderManager::CompileShader(const std::wstring& filePath, const wchar_t* profile, IDxcUtils* dxcUtils, IDxcCompiler3* dxcCompiler, IDxcIncludeHandler* includeHandler)
{
	...
	LogString::Log(LogString::ConvertString(std::format(L"Begin CompileShader, path:{}, profile:{}\n", filePath, profile)));
	...
	{
		LogString::Log(shaderError->GetStringPointer());
	...
	// 成功したログを出す
	LogString::Log(LogString::ConvertString(std::format(L"Compile Succeeded, path:{}, profile:{}\n", filePath, profile)));
	assert(SUCCEEDED(hr));
